 ...
 * @since 1.0
 */
class Dentist_Contact extends Widget_Base {

    public function get_name() {
        return 'dentist-contact';
    }

    public function get_title() {
        return __( 'Contact', 'dentist' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            'contact_map',
            [
                'label' => __( 'Map Settings', 'dentist' ),
            ]
        );
        $this->add_control(
            'map_switch',
            [
                'label' => __( 'Show Map', 'dentist' ),
        ...
        $this->add_control(
            'map_address',
            [
                'label'         => esc_html__( 'Address', 'dentist' ),
                'type'          => Controls_Manager::TEXT,
                'label_block'   => true,
                'default'       => esc_html__( 'Rabindra Sorobor Dhanmondi Lake Rd', 'dentist' ),
                'description'   => esc_html__( 'Before use map widget make sure you are set google map api key from customizer -> theme options -> general -> google map api key', 'dentist' )
            ]
        );
        $this->add_control(
            'map_lat',
            [
                'label'       => esc_html__( 'Latitude', 'dentist' ),
                'label_block' => true,
                'type'        => Controls_Manager::TEXT,
                'default'     => esc_html__( '23.745227', 'dentist' )
            ]
        );
        $this->add_control(
            'map_lng',
            [
                'label'       => esc_html__( 'Longitude', 'dentist' ),
                'label_block' => true,
                'type'        => Controls_Manager::TEXT,
                'default'     => esc_html__( '90.377155', 'dentist' )
            ]
        );
        ...
        $this->start_controls_section(
            'contact_info',
            [
                'label' => __( 'Contact Info', 'dentist' ),
            ]
        );

        $this->add_control(
            'info', [
                'label'         => __( 'Create Contact Info', 'dentist' ),
                ...
                'fields'  => [
                    [
                        ...
                        'label_block' => true,
                        'type'        => Controls_Manager::TEXT,
                        'default'     => esc_html__( 'Dhaka, Bangladesh', 'dentist' )
                    ],
                    [
                        'name'    => 'desc',
                        'label'   => __( 'Contact Descriptions', 'dentist' ),
                        'type'    => Controls_Manager::WYSIWYG,
                        'default' => esc_html__( 'Write something...', 'dentist' )
                    ],
                    [
                        'name'  => 'icon',
        ...
        $this->add_control(
            'color_title', [
                'label'     => __( 'Address Title Color', 'dentist' ),
        ...
        $this->add_control(
            'color_desc', [
                'label'     => __( 'Address Description Color', 'dentist' ),
                'type'      => Controls_Manager::COLOR,
                'default'   => '#777',
                'selectors' => [
    ...
    wp_enqueue_script('dentist-map-custom');
                    ...
                    ?>
                        <div class="single-contact-address d-flex flex-row">
                            ...
                            <div class="contact-details">
                                ...
                                //
                                if( ! empty( $info['desc'] ) ) {
                                    echo dentist_get_textareahtml_output( $info['desc'] );
                                }
                                ?>
                            </div>
                        </div>
