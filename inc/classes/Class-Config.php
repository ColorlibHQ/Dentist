...
final class Dentist {
	...
	private function setup() {
		...
		$enqueu = new Dentist_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->dentist_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'dentist_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'dentist', DENTIST_DIR_PATH . '/languages' );
		...
		add_image_size( 'dentist_widget_post_thumb', 100, 60, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'dentist-np-thumb', 60, 60, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'dentist' ),
            'social-menu'   => esc_html__( 'Social Menu', 'dentist' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = DENTIST_DIR_CSS_URI;
		$jsPath  = DENTIST_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					'handler'		=> 'dentist-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->dentist_version,
				),
				array(
					'handler'		=> 'dentist-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'dentist' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate dentist theme customizer
		$dentist_theme_customizer = new dentist_theme_customizer();
	}
	...
	public function set_elementor_flag() {
		update_option( 'dentist_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'dentist-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'dentist_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'dentist-elementor-default-style',  DENTIST_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->dentist_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'dentist_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'dentist-elementor-notice', DENTIST_DIR_JS_URI.'dentist-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'dentist-elementor-notice',
				'dentistElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'dentist-elementor-notice-nonce' ),
				)
			);
		}
