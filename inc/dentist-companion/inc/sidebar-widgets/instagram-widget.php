...
class dentist_instagram_widget extends WP_Widget {
...
'dentist_instagram_widget',


// Widget name will appear in UI
esc_html__( '[ Dentist ] Instagram Widget', 'dentist' ),

// Widget description
array( 'description' => esc_html__( 'Add footer instagram widget.', 'dentist' ), )
);
...
echo wp_kses_post( $args['before_title'] . $title . $args['after_title'] );
    ...
    $api = dentist_instagram_instance();
...
<ul class="instafeed d-flex flex-wrap">
	...
	foreach( $items as $item ){
		...
		echo '<li><img src="'.esc_url( $imgurl ).'" alt="'.esc_attr( dentist_image_alt( $imgurl ) ).'"></li>';
	}
	?>
</ul>
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'INSTAFEED', 'dentist' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'itemnumber' ) ); ?>"><?php esc_html_e( 'Item Number:' ,'dentist'); ?></label>
...
function dentist_instagram_load_widget() {
	register_widget( 'dentist_instagram_widget' );
}
add_action( 'widgets_init', 'dentist_instagram_load_widget' );